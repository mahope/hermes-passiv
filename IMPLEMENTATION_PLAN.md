...
- `DEPLOY`: `**DEPLOY OK b163893** — den åbne note fra opgave 71 er lukket målt på indhold, ikke på HTTP 200. \`/build-info.json\` bærer \`b163893\` på mahope.tools, cleancopy.tools og deskuptime.com (\`sitemap_count\` **252** uændret; bugbottle.dev har ingen build-info, uændret). Live \`/page-profile\` og \`/da/page-profile\` har hver **1** kassalink, **6**/**7** Gratis-\`<li>\` (de 5 erklærede + 1/2 rent praktiske) og **5** Pro-\`<li>\`, og alle **ti** erklærede labels ligger i Gratis-kortet og **ingen** i Pro-kortet — dvs. den nye regel er overholdt i live, ikke kun i kilden. **VERIFICÉR DEPLOY: \`check_free_features\` kræver nu at en erklæret gratis-funktion står i Gratis-kortet aad2295 26/9 15:25 UTC (kørsel 36251902287, stående i kø ved merge)** — samme forhold som ovenfor: \`tools/**\` er i path-filteret, kørslen stod i kø ved merge og skal blive grøn, men **intet domæne ændrer indhold**. Verificér på indhold: build-info bærer merge-kommitten med \`sitemap_count\` 252, CI grøn med \`stripe-ctas\` på **13 købssider / 0 fejl** og selftesten på **39/39** (et lavere tal betyder at den nye blok ikke kom med), og de to page-profile-sider er uændrede med 1 kassalink hver.`
