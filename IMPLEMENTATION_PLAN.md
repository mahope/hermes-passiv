- `ITERATION_ID`: `frit-funktioner-i-katalog-2026-09-26`
- `STATE`: `Opgave 71 FÆRDIG — NEXT_TASK (1) fra opgave 70 er løst: **de fire gratis-funktioner, der lå som "ja/ja" i begge købssiders tabel, er nu erklæret i katalogen**, så porten kan se dem blive flyttet op i Pro-kortet. **Fund 1 — de fire er målt frie i *samme* kode, og fri-stien har præcis én `require_pro`.** `main()`'s frie sti er `page-profile/page_profile.py:1079-1103`, og der er **én** `require_pro` i den: `:1094` på `--html-report`. Alt hvad der sker før eller efter det uden for dette ene kald er gratis. Derfor: **alle tjek** — `:1085 analyze(args.url, html, headers)` → `analyze :408-464`, ingen `require_pro` mellem `:1080` og `:1085`; **rapport med score og karakter** — `:1086 score(result)` → `score :465-577` og `:1103 format_terminal(...)` → `format_terminal :593-750`; **JSON-output** — `:1101 print(format_json(...))` → `format_json :751-784`; **redirect-kæde** — `:1080 fetch_page` bygger `redirect_chain :363-402`, trykt af `format_terminal :727-731` og `format_json :755`. **Fund 2 — siderne har hele tiden sagt det rigtige, så det var katalogen der var ufuldstændig, ikke teksten.** Målt med portens egen `TierCards` på begge rigtige filer: alle **otte** nye labels (4 × 2 sprog) ligger i **Gratis-kortet**, og **0** ligger i Pro-kortet. Dertil at alle fire står som "ja/ja" i Gratis-vs-Pro-tabellen på begge sider. Det er altså ikke en side der løj (som opgave 69 fandt) — det er fire gratis-funktioner, ingen holdt øje med, fordi kun historik var skrevet ned. **Fund 3 — selftestens "én gang pr. produkt"-assert tællede *poster*, ikke *indgange*, så den har aldrig testet andet end ettemalssagen.** Den krævede `len(missing_where) == 1`, hvilket kun holder for én `free_features`-post — altså præcis den ene `history` fra opgave 70. Den døde ved min femte post. Rettet til at dømme tre ting samtidig: én linje pr. post, alle linjer i **ét** produkt, og **ikke** mere end én linje pr. post — fordi produktet har **to** købssider, så en port der meldte pr. tilbud ville have givet 10 i stedet for 5. Det er den egenskab asserten egentlig ville forsvare, og den lå i et tal der så ud til at gøre det. **Fund 4 — de fire nye poster får ti selftest-scenarier, fordi en port der læser ingenting også er grøn.** Hver post × begge sprog: den rigtige Gratis-sætning flyttes ind i Pro-kortet på den rigtige købsside, og porten skal ramme præcis den posts \`id\`. **Fund 5 — isolationen er målt, ikke håbet.** Jeg gjorde porten *blind* for én post ved at erstatte dens labels med \`__blind__\` (min første isolation fjernede posten i stedet, hvilket ramte Fund 3s assert først og bevisede intet om den nye blok). Resultat: selftesten fejler med præcis *"redirect-chain på site/da/page-profile.html; redirect-chain på site/page-profile.html"* og intet andet. Uden den ville den nye blok have været ti linjer der aldrig kan fejle. **Fund 6 — den anden retning mangler stadig, og det er nu målt i stedet for formodet.** Sletter man Gratis-kortets sætning og lader alt andet være, er porten stadig **GRØN**. Det er NEXT_TASK (3): en erklæret gratis-funktion skal *nævnes i Gratis-kortet*, ellers kan rettelsen blive "fjern påstanden" i stedet for "sig hvor den hører hjemme".`
- `ACTIVE_TASK`: `— (ingen opgave I GANG)`
- `BASELINE`: `main@9d64ca4`
- `LAST_BRANCH`: `ceo/prokortet-deklarerer-gratis`
- `NEXT_TASK`: `Køen er tom igen. **Fire ting, og den første er den anden retning i den fejlklasse opgave 69 startede.** (1) **Kræv at en erklæret gratis-funktion *nævnes i Gratis-kortet*** — det er NEXT_TASK (3) fra opgave 70, og den er målt i mangel: med dagens kode er porten grøn, selv om Gratis-sætningen er slettet fra begge sider. Nu hvor der er **5** poster pr. produkt at dømme, er reglen billig: i \`check_free_features\` kræves for hver post at dens \`labels[lang]\` findes i \`cards.text("free")\`, ikke blot at de mangler i Pro-kortet. Kravet skal være **én** label pr. sprog, ellers er det en særskilt fejlform. Beviset på den gamle kode er trivielt: de otte mutationer fra denne iteration flyttet *grå* ind i Gratis-kortet i stedet for Pro. (2) **Målingen til NEXT_TASK (2) fra opgave 70 er delvist lavet her, og den er større end opgaven troede:** \`check_free_features\` dømmer kun sider med et \`tier-card pro\`, og **7 af de 13 købssider har ikke ét** — \`clean-copy\`, \`clean-copy-tool\`, \`activate/\`, \`da/activate/\`, \`deskuptime/\`, \`da/deskuptime/\` og \`blog/desktop-website-monitor-cli\` springes alle fra på \`pro_card == ""\`. Så for de produkter hvis købsside kun har en gratis/Pro-*tabel* (EUComply har \`id="pricingSection"\`/\`id="pris"\`, Clean Copy har en sætning) dømmer hele porten slet ikke. Det er samme fejlklasse som de andre checks, der tæller både sætning og tabel, og det er derfor næste rigtige opgave: mål hvilken flade hver af de syv har, og døm den i stedet for at springe den. (3) **Mandag 28/9 06:00 UTC: \`reports/weekly/2026-40.json\` skal have \`traffic.checkout.tracked_days: 2\`** — uændret fra opgave 68, \`clicks\` er et målt 0. (4) **❓ Search Console (❓ 1), de otte produkter uden købsside (opgave 58) og ❓ 14 fra opgave 47 (testlicens til live-verificering af EUComply-rapporten) er uændrade** og kræver Mads. **(5) NEXT_TASK (6) fra opgave 70 er stadig en måling:** om alle 13 \`offers\`-ruter + 4 \`core_pages\` matcher \`normalizeTrackedPath\`. Skriv den kun ned hvis den finder flere.`
- `GATE`: `GRØN — \`python3 tools/quality_gate.py\`: **GRØN, 47 steps** (uændret — ingen ny regel, kun eksisterende input i \`tools/\`, så workflowens path-filter er urørt). \`tools/check_stripe_ctas.py\`: **13 produkter / 13 dokumenterede købssider, 0 fejl**; \`--self-test\` **39/39 fejlformer** (uændret i antal — de nye asserts er et selvstænd blok i \`free_features\`-scenariet, ikke nye nummererede fejlformer) med **ét korrigeret assert og ét nyt blok på 10 scenarier**. Missionens egne fire kørt enkeltvis og målt i gaten: \`build_sites.py\` grøn (**321 filer / 252 sitemap-ruter / 0 brudte** på mahope.tools, 33/5/7 på de tre andre, **0 brudte** alle steder), \`seo_check.py\` **310 sider 0 fund**, \`stripe-worker\` **155/155** uændret, \`check_inline_js.py\` **299 filer 0 problemer** uændret. **Bevis på den rigtige kode, tre former.** (a) **Åtte mutationer på de rigtige filer** — den rigtige Gratis-sætning flyttet ind i Pro-kortet på hver af de fire poster i begge sprog — giver **præcis 1 rød** hver, og hver rammer den post \`id\` den skal. (b) **Katalogfejl:** alle fem poster uden \`where\` giver **5 røde i ét produkt** (10 ville være pr. købsside), og en post uden \`labels.da\` giver **1 rød** i stedet for grøn. (c) **Isolation:** porten gjort blind for én post giver selftestfejl med præcis den posts \`id\` på begge sider. **Negativ kontrol:** Gratis-kortet må nævne alt, og \`Alt i Gratis, plus:\` i Pro-kortet er ikke en fejl. **\`.gitignore\`-urørt, \`dist/\` uændret (gitignored), \`site/\` urørt — ingen eneste byte på en publiceret fil, og \`site/_worker.js\` urørt pr. missionens egen regel (\`/api/stripe-webhook\`, \`/api/stripe/fulfillment\`, \`/api/download\` urørt).`
- `SLIP`: `Ingen på indhold, ~35 min — under dræbningen, men kun fordi jeg committede koden med det samme da gaten blev grøn. Jeg sprang reviewen over som kontrakten tillader: 61 indsatte linjer i 2 filer, og beviset er otte mutationer på de rigtige købssider plus en isolation, ikke en læsning. **Fire ærlige ting om hvor tiden gik.** (a) **Mine første to mutationer sagde "GRØN" på en port der ikke var brudt — fordi mit eget harness var brudt, to gange.** Først lukkede \`run(pairs)\` over den *urettede* katalog og ignorerede den muterede, så selv \`where = "   "\` på alle fem poster gav 0 røde. Så indsatte jeg \`<li>\` lige før \`class="tier-card pro"\`, altså *inde i* \`<div\`-taggen, så \`TierCards\` aldrig åbnede kortet. Begge fejl så ud som "porten virker ikke". **Det, der reddede mig, var en positiv kontrol:** "alle 5 uden \`where\` → 0 røde" er et resultat, der ikke kan være rigtigt, fordi den samme kørsel på den rigtige katalog giver 5. Uden den kontrol havde jeg skrevet fire poster i planen som om de var dømte af en port, der aldrig så dem. Det er **fjerde** svarhert i denne plan på en grøn port, der var grøn af en fejl grund, og det er nu tre gange samme grund: harnesset læste ikke noget. (b) **Min første isolation var for blunt** — jeg fjernede posten i stedet for at blinde den, og den ramte Fund 3s nye assert, som så beviser intet om den nye blok. Løsningen var at erstatte *labels* med \`__blind__\`, så posten stadig findes og tælles. (c) **Jeg brugte ~8 minutter på at finde ud af, hvorfor porten ikke dømte, og det var ikke porten.** Jeg læste funktionen to gange, kørte \`sys.settrace\` på den, og fandt først så at sporet gik hele vejen gennem linje 1322 med en ikke-tom \`pro_card\` — altså at porten var korrekt og mit input var ødelagt. Sporet var det hurtige; hvis jeg havde tænkt "hvad er \`by_path\`-nøglen" først, havde det været det samme på ti sekunder. (d) **Jeg skrev planen til sidst i stedet for underveis** — bevidst, fordi kontrakten siger at planopdateringer skal committes sammen med den kode de beskriver, så en ren plan-commit ville være den anden i rækken. **Én ting jeg bevidst lod ligge:** de 7 købssider uden \`tier-card pro\` (NEXT_TASK (2)) er målt, men ikke rettet — de hører til en opgave der kræver at måle hvilken flade hver af dem har, og det er ikke en konsekvens af denne diff.`
- `TASK_ATTEMPTS`: `36: 1/1, 47: 1/1 (afvist på målt grundlag), 48: 1/1, 49: 1/1, 50: 1/1, 51: 1/1, 52: 1/1, 53: 1/1, 54: 1/1, 55: 1/1, 56: 1/1, 57: 1/1, 58: 1/1, 59: 1/1, 60: 1/1, 61: 1/1, 62: 1/1, 63: 1/1, 64: 1/1, 65: 1/1, 66: 1/1, 67: 1/1, 68: 1/1, 69: 1/1, 70: 1/1, 71: 1/1`
- `DEPLOY`: `VERIFICÉR DEPLOY: de fire gratis-funktioner er nu erklæret i katalogen, så Pro-kortet ikke kan sælge dem b163893 26/9 ~17:4x UTC` — GitHub Actions deployer med det samme (`tools/**` er i path-filteret), så denne iteration får grønne `gate`-steps, **men intet domæne ændrer indhold**: kun `tools/stripe_catalog.json` og `tools/check_stripe_ctas.py` rørtes, og ingen publiceret fil har ændret en byte. Verificér derfor på **indhold, at intet er ændret** — HTTP 200 beviser intet her: `https://mahope.tools/build-info.json` skal bære kodecommitten `5ab6f21` på alle tre domæner med `sitemap_count` **252** (uændret), CI skal være grøn med `stripe-ctas` på **13 købssider / 0 fejl** og selftesten på **39/39** (et lavere tal betyder at den nye blok ikke kom med), `/page-profile` og `/da/page-profile` skal hver stadig have **1** kassalink og uændrede kort (**5** Gratis-punkter, **5** Pro-punkter), og `/compliance-report` + `/da/compliance-report` skal være uændrede. Opgave 70s egen state er samtidig bekræftet: kørsel `36249218333` for `9d64ca4` er **grøn**.
