...
- `DEPLOY`: `— (ingen åben note) Denne iteration rører **kun \`tools/stripe_catalog.json\` og \`tools/check_stripe_ctas.py\`**, som ikke publiceres, så intet domæne ændrer indhold — en kunde kan ikke se forskellen. \`tools/**\` er i workflowens path-filter, så kørselen får alligevel grønne \`gate\`-steps. Verificér på indhold, at intet er ændret: \`https://mahope.tools/build-info.json\` skal bære kodecommitten \`5ab6f21\` på alle tre domæner med \`sitemap_count\` **252** (uændret), CI skal være grøn med \`stripe-ctas\`-stepet på **13 købssider / 0 fejl** og **39/39** i selftesten, og \`/page-profile\` + \`/da/page-profile\` skal hver stadig have **1** kassalink og hver **5** Gratis-punkter og **5** Pro-punkter uændret.`
