...
- `DEPLOY`: `VERIFICÉR DEPLOY: produkt→motor-kortet — hvert regel-løfte måles mod den motor siden sælger, og 31 forkerede tal er rettet (branch ceo/produkt-motor-kort, merge 4d51edd) 26/9 ~20:56` — GitHub Actions deployer med det samme (\`site/**\` og \`tools/**\` er i path-filteret), og diffen rører **14 publicerede sider** heraf **12 guidesider** og **to købs-/downloadsider**, så den skal verificeres på indhold, ikke på HTTP 200. Verificér:
   ...
   - \`https://mahope.tools/guides/craftcms-accessibility-check\` skal have **0** forekomster af \`16 WCAG 2.1 AA rules\` og **3** af \`15 WCAG 2.1 AA rules\`. Samme mål på de elleve øvrige guidesider; \`webflow\` har 2, \`squarespace\` 1, resten 3. Tæl på hele HTML-kilden, ikke på synlig tekst — kun heroen er synlig.
   - \`https://mahope.tools/blog/eaa-compliance-scanner-desktop\` skal have **1** af \`same 22 WCAG 2.1 AA rules as our CLI\`, **0** af \`as our CLI and web-based scanner\`, og **1** af \`22 WCAG 2.1 AA rules\` i alt. Det sidste tal skal være **1**: en forekomst mere betyder at en sætning er dublet.
   - \`https://mahope.tools/downloads\` skal have **0** af \`as the CLI and web versions\`, **1** af \`as the CLI,\` og **1** af \`The browser scanner\` med \`15 of them\`. Sidste del er den nye, ærlige sætning.
   - \`https://mahope.tools/blog/free-accessibility-testing-tools\` skal have **1** af \`Ours scans 22 WCAG 2.1 AA rules per page\`.
