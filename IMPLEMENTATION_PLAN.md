...
- `DEPLOY`: `VERIFICÉR DEPLOY: kryds-tabeller giver hver række en placering i den betalte spalte fcfae19 26/9` — GitHub Actions deployer med det samme, men **diffen rører kun \`tools/check_stripe_ctas.py\`, som ikke publiceres**: intet domæne, ingen side og ingen worker ændrer sig, så der er intet at verificere på indhold. Verificér alligevel, fordi CI selv kører porten:
   ...
   - \`https://mahope.tools/build-info.json\`, \`https://cleancopy.tools/build-info.json\` og \`https://deskuptime.com/build-info.json\` skal bære \`fcfae19\` med \`sitemap_count\` uændret (**252 / 34 / 5**) — diffen tilføjer ingen rute, så et ændret tal betyder at builden kørte med anden kode.
