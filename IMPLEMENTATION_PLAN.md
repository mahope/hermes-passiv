...
- `STATE`: `Opgave 27 I GANG — forudsætningen var delvis forkert. Kun **Firefox**-README'en ligger inde i et publiceret arkiv (\`FIREFOX_EXTRA = ["LICENSE", "README.md"]\`); **Obsidian**-arkivet består af main.js, manifest.json, styles.css og version.txt, så \`obsidian-plugin/README.md\` har aldrig været låst af en version. Den kunne have været rettet i denne iteration, uafhængigt af enhver udgivelse. Firefox-README'en derimod har en **fejl kunden kan se**: badge på \`version-1.4.1\`, mens arkivet den henter hedder \`clean-copy-firefox-v1.5.3.zip\` og manifesten siger 1.5.3. Den ligger i samme mappe som kilden, ingen gate læser den, og den er den README butikslisten viser.`
...
- `ACTIVE_TASK`: `27 — de shippede README'er skal kunne nævne Pro`
- `NEXT_TASK`: `(denne iteration)`
