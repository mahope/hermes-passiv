...
- `STATE` (før): `Opgave 39 FÆRDIG — den slettede 1.3.3-kildearkiv ligger nu bag en 301, og porten kan se en sletning overhovedet. **Fund 1 — opgavens forudsætning var for smal, og jeg målte den i stedet for at tro den.** Den sagde "1.3.3-arkivet" som om den var den eneste. \`git log --diff-filter=D -- site/downloads\` giver **15** slettede filer, ikke én. Jeg målte dem alle med \`curl -sI\` på begge domæner: **præcis én** svarer 200, \`mahope.tools/downloads/eaa-scanner-desktop-src-1.3.3.zip\` — den opgave 38 havde navngivet. De fjorten andre er 404 på cleancopy.tools *og* mahope.tools. Konsekvensen er en scope, jeg ville have fejlet på uden målingen: jeg tilføjede **én** 301, ikke tolv spekulative. En 301 på en fil ingen kan hente løser intet og ville have været tolv nye regler i produktionskritisk kode uden en kunde. De fjorten er dog ikke glemt — de står i \`_gone\` med målingen, så listen er data i én fil. **Fund 2 — planens egen note om porten var rigtig, og jeg fandt præcis hullet.** Den skrev at json, worker og dist var enige "fordi sletningen ikke er opført nogen steder". Det er hele pointen: de tre kilder læser *nuet*, og en sletning er en *ændring*. Ny kontrol 6 \`undocumented_deletion\` læser \`git log --diff-filter=D\`, så porten ser sletninger i stedet for kun tilstande. Beviset kommer fra rigtig historik, ikke fra en konstrueret streng: scenariet sletter \`mahope.tools\`-blokken fra json og fanger præcis \`/downloads/eaa-scanner-desktop-src-1.3.3.zip\`. **Fund 3 — min første skrivning af katalog-json'en slettede \`cleancopy.tools\`-blokken.** Jeg skrev hele filen i stedet for at redigere den, og den blok holder 1.5.3-301'en fra opgave 28. Følgen var \`orphan_worker_rule\` med det samme: workerens regel stod uden json. Porten fangede den i samme kørsel som den så den nye regel, og den gamle 1.5.3-regel havde været død i den første commit jeg lagde. Samme fejlform som opgave 30 fund 1, som et overskrivningstab. **Fund 4 — min egen selftest lå på position, og blev en tavs no-op.** Mutationerne skrev \`"...v1.5.4.zip',\\n    };"\`, altså den *sidste* linje før \`};\`. Da 1.3.3-regelen blev føjet til, holdt mønstret ikke længere, mutationerne gjorde ingenting, og selftesten meldte **"selvrefererende redirect (forventede json_worker_mismatch, fik [])"** — altså afslørede den at den aldrig havde testet noget for den fejl. Nu skriver \`set_worker_target(text, path, target)\` reglen efter *stien*, ikke efter positionen. Selftest 11 → 19 kontroller. **Fund 5 — den nye kontrol ville være teater i præcis sit eget tilfælde.** En klon med \`depth=1\` har ingen slettede filer i historikken, så \`undocumented_deletion\` ville være grøn ved at læse ingenting. Derfor tæller den nu \`git rev-list --count HEAD\` og fejler under to commits med en fejl der *navngiver* årsagen, og selftesten monkeypatcher læseren til en grund klon og kræver \`history_unreadable\` — efterfulgt af en negativ kontrol der gendanner den og kræver grønt igen. Det er samme krav som opgave 17 fund 3: siger porten intet om sin egen evne, skal den sige det med en fejl. **Fund 6 — worker-beviset er på den gamle kode, ikke på en streng.** Den falske \`ASSETS\` i \`tests/stripe-worker.test.mjs\` serverer nu 1.3.3 med den rigtige løgn ("$19/year … Purchase a license at hermes-passiv.pages.dev/clean-copy"). Uden reglen: **200, løgnen i kroppen, og stien tælles som download i KV.** Med reglen: 301, ingen løgn, ingen tælling. Stripe-worker 77 → **83/83**. **Bemærk om rækkefølgen:** reglen slår \`/downloads/\`-ruten, ellers ville \`handleDownload\` tælle den gamle fil og den falske tekst ville blive serveret videre — samme grund som opgave 28, og derfor ligger den stadig samme sted.`
- `STATE` (før): `Opgave 37 FÆRDIG — researchiterationen (opgave 36 kræver Mads) fandt den hidtil værste udgave af den fejlklasse, opgave 26, 29, 30, 31 og 35 har jaget fem gange: **en påstand om noget der ikke findes — her en PRIS.**
- `STATE`: `Opgave 37 FÆRDIG — researchiterationen (opgave 36 kræver Mads) fandt den hidtil værste udgave af den fejlklasse, opgave 26, 29, 30, 31 og 35 har jaget fem gange: **en påstand om noget der ikke findes — her en PRIS.** `site/blog/eaa-compliance-scanner-desktop.html` havde `Price | Free (MIT) | $19/year` i sin Free/Pro-tabel og **0 betalingslinks på hele siden** (udmålt, ikke antaget). Der findes ingen `product_key` for EAA-scanneren, ingen knap, ingen checkout. **Fund 1 — siden modsagde sig selv 39 linjer nede:** eget Licensing-afsnit sagde *"There is no Pro licence for the EAA scanner today, and no price to pay for one"*, og `site/downloads.html` sagde det samme. To sider i samme produkt modsagde hinanden, og bloggen var den, der løftede en pris. **Fund 2 — min egen plan lod opgaven ligge med en forudsætning, der var forkert.** Opgave 31 skrev at Pro-cellerne ikke kan fyldes fordi produktet ikke står i kontrakten, og meldte derfor opgave 32 `BLOCKED: kræver Mads`. Men de reelle tal findes tre steder i repoet (`downloads.html:109`, `desktop/main.js:132`, bloggens egen bulletliste). **Det der ikke kan udledes, er kun prisen** — og en pris på en vare, der ikke eksisterer, skal ikke udledes, den skal fjernes. Det gjorde opgaven til en tekstopgave, ikke en produktafgørelse; ❓ punkt 12 (om Mads vil *oprette* produktet) står uændret. **Fund 3 — de tomme celler var ikke sløshed, men en bivirkning af en god oprydning.** `2c9909d` ("Ryd emoji-ikoner…") fjernede `✓` fra tabellen; de tre celler der kun *indeholdt* et `✓`, blev tomme, mens `✓ Unlimited` bare mistede symbolet. Samme fejlform som opgave 30 fund 1. Derfor er cellerne nu ord (`yes`, `planned`, `Not for sale yet`) frem for symboler, så en fremtidig oprydning ikke kan tømme dem igen. **Fund 4 — porten var grøn på præcis den fejl, den skulle fange, fordi den kun læste katalogens købssider.** `check_free_tier` gennemgår `catalog["offers"]`, og EAA-siden er ikke et tilbud, så ingen regel læste den — samme hullet som opgave 26 (rod-README) og opgave 35 (`reports/weekly/`). Ny `check_comparisons(catalog, source_pages())` læser *alle* sider: ingen tomme celler i en synlig gratis/Pro-tabel, og **en Pro-pris kræver et betalingslink på samme side**. Bevis på de rigtige gamle filer fra `git HEAD`: 4 problemer (3 tomme celler + `$19/year`), mod den rettede fil 0, mod hele treeet 0. **Fund 5 — min egen første portversion ville have været grøn på `/clean-copy-tool`.** `PRICE_TOKEN` er `\$\s?\d[\d.]*` og kan kun se et `$`, men sidens Pro-pris celle skriver **"19 USD per year"**. Bevis: med kun `PRICE_TOKEN` er der **0** priser at finde i hele familiens vigtigste købsside; fjerner man dens købsknap, ville porten være grøn på præcis den manglende købsmulighed den er skrevet til at fange. Ny `CURRENCY_AMOUNT` tæller `$19`/`€19`/`£19`/`19 USD`/`19 kr`, og selftesten sigter eksplicit på forskellen, så et scenarie der kun virker med `$` ikke kan stå som fanget. **Fund 6 — generatoren skrev den værre version end den publicerede side:** `make_blog_desktop_en.py` skrev `"$19/year (coming soon)"` i tabellen og *"Pro requires an annual license key ($19/year)"* i Licensing, hvor den publicerede side allerede havde den ærlige sætning. Kun en rettelse i `site/` ville være gået tabt, præcis som opgave 30 fund 1. Begge er rettet. **Konvertering:** den direkte effekt er lille og ærlig — der var ingen købsknap at miste. Den vigtige effekt er at en citatpris er væk fra en publiceret side, så EAA-scanneren kan sælges uden at nogen beskyldes om at love noget. Samme øjeblik en `product_key` kommer i katalogen, bliver `$19/year` i tabellen lovlig, fordi siden så skal have et betalingslink. **Worker urørt:** `site/_worker.js` ikke ændret, stripe-worker uændret 77/77. **Bemærk:** den slettede 1.3.3-arkiv mangler en 301, se opgave 39 — indtil den er der, kan den gamle kode stadig hentes fra CDN'en.`
...
- `STATE` (før, opgave 30): `Opgave 30 FÆRDIG — den samme påstandsklasse som opgave 2, 27 og 29, i guide-siderne. **Planens forudsætning var delvis forkert, og de to sider den mente var fejl, er sande.** Den skrev "fem publicerede FAQ'er", men to af dem kan ikke rettes. (a) `copy-table-website-iphone-ipad` løfter at *bookmarklet'en* kører entirely in browser, og den shippede `site/clean-copy-bookmarklet.js` har nul `fetch`/`XMLHttpRequest`/`sendBeacon` — den er to linjer ren injiceret JS. Løftet er præcist og sandt. (b) `accessibility-scanner-cli` siger "the scanning happens entirely on your machine", og `scanner/scanner_core.py` henter kun den URL den bliver bedt om, med ingen reference til mahope.tools — næste FAQ-kort siger det samme rigtigt. **At "rettelse" de to ville have været over-korrektion på sande påstande.** De fire reelle fejl lovede at Clean Copy "works entirely inside your browser" / "everything runs locally in your browser", mens siden selv pinger `/api/track` (sidevisningstal + CTA-beacon) og en Pro-nøgle tjekkes online. **Fund 1 — tre af de fire skrives af en generator, så rettelsen i `site/` alene var nok til at gå tabt.** `make_blog_sheets_en.py:191`, `make_blog_notion_en.py:203` og `make_hub_copy_clean.py:164` skriver deres side ved `open(out,'w')` og dør så på `raise SystemExit` — de *kan* skrive den gamle tekst tilbage. Airtable-siden er derimod håndskrevet; ingen generator i `tools/` nævner den, kun som link. **Fund 2 — min egen port fangede en rigtig forglemmelse i samme iteration.** Jeg havde rettet `make_hub_copy_clean.py` men ikke `site/copy-clean-guide.html`, den skriver: 6 fejl. Genbrug af en eksisterende port på en ny filklasse fangede altså en fejl i mit eget arbejde, før commit. **Fund 3 — et generatorløfte delt på to Python-strenge er usynlig for grep.** Den gamle hub-sætning var `'...Nothing is ' 'uploaded until you paste...'`, så ingen tekstsøgning kunne have fundet den. `check_generator` læser derfor generatorens strenge via `ast`, som folder sammensatte literals ind i én `Constant` — uden at køre generatoren, som ville skrive filer. **Fund 4 — min egen mutation var grøn fordi den intet muterede.** Selftestens "generatoren uden nogen afsløring" erstattede `'an anonymous page-view count'`, som også var delt på to linjer, så `str.replace` ramte nul steder og scenariet stod som *ikke fanget* — tilfældigt, fordi resten af porten var rød. Scenariet fejlede ærligt i stedet for at lyve, og der er nu en assertion på at mutationens anker findes.`
- `STATE` (før): `Opgave 29 FÆRDIG — researchiterationen fandt den tredje udgave af den fejlform, opgave 2 (DeskUptime) og opgave 27 (shippede tekster) allerede havde fundet to gange: et absolutt privatlivsløfte på en flade, hvor det er modbevist af samme dokuments egen kode. **Fund 1 — den værste flade var `/clean-copy-tool`.** Det er siden hvor Clean Copy Pro **købes** (Stripe-knappen) og **aktiveres** (`fetch('/api/license/activate')`, linje 589). 46 linjer over den knap stod en privatlivsnote direkte under H1: *"**Nothing leaves your device.** The conversion runs in JavaScript on this page. No upload, no server, no cookies, no tracking of your text."* Den er ikke bare unøjagtig — den er modbevist af dokumentet selv, og den stod oveni den betalingsknap der skal få en Pro-køber til at taste sin nøgle ind. **Fund 2 — opgave 27 rettede samme sides *søskende* for en halv.** På `/clean-copy` blev FAQ'et ("One exception: if you activate a Clean Copy Pro license key…") og faktabladsrækken rettet, men **hero-noten lige over Install-knappen blev stående**: *"Runs entirely on your machine — no account, no server, no data sent anywhere."* To sider af samme dokument, der siger modsatte ting, og den forkerte stod mest synligt. DA-versionen havde den samme fejl. **Fund 3 — ingen port læste fladerne.** `check_product_copy` fandt 5 kilder, alle DeskUptime. Ingen af de tre Clean Copy-filer var i portens læseliste, så hele klassen var uden gaten. **Fund 4 — min egen selftest var grøn på den mutation, den skulle fange.** Selftestens JSON-LD-scenarie indsatte en *ny* `description`-nøgle i et objekt, der allerede havde én; `json.loads` tager den sidste, så den nye tekst blev overskrevet af den ældre, og scenariet stod som fanget uden at prøve noget. Nu muteres den eksisterende værdi. Samme fejlform som opgave 23, 26 og 28: en port der er grøn på præcis den fejl, den er skrevet til at fange.`
- `STATE` (før): `Opgave 27 FÆRDIG — forudsætningen var delvis forkert, og den falske del var større end opgaven så. Kun **Firefox**-README'en ligger inde i et publiceret arkiv (\`FIREFOX_EXTRA = ["LICENSE", "README.md"]\`); **Obsidian**-arkivet består af main.js, manifest.json, styles.css og version.txt, så \`obsidian-plugin/README.md\` har aldrig været låst af en version og kunne have været rettet længe før nu. **Fund 1 — den README, der SKAL rettes, lovede noget der ikke er sandt.** Den sagde "**No network requests** — nothing leaves your browser", men \`license.js\` POSTer nøglen til \`https://mahope.tools/api/license\`. Samme løfte stod tre steder på cleancopy.tools (meta description, faktablads-rækken, FAQ) og i Obsidian-README'en ("No network access", mens \`main.js:542\` gør præcis det samme). Det er den påstand opgave 2 var nødt til at trække tilbage på DeskUptime — nu på den mest distribuerede klient, i den fil butikslisten viser. **Fund 2 — den havde også en gammel udgave på badge'et.** \`version-1.4.1\` i et arkiv, der hedder \`clean-copy-firefox-v1.5.3.zip\`, mens manifesten siger 1.5.3. **Ingen gate læste den**: \`check_versions\` læser manifesten, \`check_pages\` læser \`site/\`, og byte-sammenligning af arkivet kan ikke vide at *teksten* er forældet. Ny check i \`check_clean_copy_distribution.py\`, 26 → 27 fejlformer. **Fund 3 — min egen første version af checken var grøn på præcis den fejl, den var skrevet til at fange.** Jeg gav den en \`SHIPPED_HISTORICAL_VERSIONS\` med 1.4.1, fordi README'en nævner "Table column alignment preserved (v1.4.1)" som feature-historik — og så var undtagelsen skrevet efter *tallet* og ikke efter *påstanden*, så det gamle badge slap igennem. Beviset stod i min egen selftest: scenariet "en README, der nævner en udgave som ikke står i historikken" blev **ikke** fanget. Løsningen er linjescoping, ikke en undtagelsesliste: en påstand er et versions-token på en linje der *nævner* en udgave, og historikken nævner ikke ordet. Nu kan ingen undtagelse skjule en påstand, fordi der ikke er nogen. **Bevis på den rigtige gamle fil:** den README fra git \`03751ae^\` lagt i et 1.5.4-arkiv giver præcis 1 fejl, "clean-copy-firefox-v1.5.4.zip/README.md siger version 1.4.1, men udgaven er 1.5.4". **Konvertering:** Firefox-README'en har nu en Pro-sektion med betalingslinket, de fem enheder (verificeret i \`options.html\` og \`_worker.js:2880\`) og en aktiveringsvej; begge README'er har donationslink. **Beslutning:** Firefox 1.5.3 → 1.5.4, arkivet bygget om, de fire sider der linker på det rettet (href *og* linktekst — den viste stadig 1.5.3). Chrome-extensionen er urørt på 1.5.3, fordi den ikke er ændret. Ingen git-tag, ingen release, ingen upload: det er Mads'.`
- `STATE` (før): `Opgave 26 FÆRDIG — fund 3 fra opgave 23 er nu lukket: rod-README'en lå uden for alle 38 gatestræk, så den fejl, opgave 23 rettede ved hånd, holdt kun fordi en researchiteration tilfældigvis læste den. Ny port \`tools/check_repo_readme.py\` med \`--self-test\`, to nye steps (**38 → 40**). Den fanger fire fejlformer: rod-README der ikke linker til nogen af de fire domæner, en bygget sti der ikke findes i route_inventory, et købslink uden for katalogens allowlist eller en pris der ikke er produktets, og en lukket udbyder nævnt som om den virker. **Bevis på den gamle, rigtige fil fra git (\`7aef580^\`): 1 fejl**, netop \`no_domains\`. **Fire fund, hvor tre var fejl i min egen port.** (a) \`no_domains\` ledede efter domænenavnet i *teksten* — og den gamle README nævner \`mahope.tools/api/license\` i en privatlivssætning, så porten var grøn på præcis den fejl, den blev skrevet til at fange; nu kræver den et link til en rigtig rute. (b) Sti-sammenligningen manglede foranledende skråstreg, så \`/books\` (i README) og \`/books/\` (i sitemapet) ikke var ens, og porten ville have fundet fejl i *alle* links. (c) Negationen \`"old "\` var et substring og fangede derfor \`"keys are **sold** at lemonsqueezy.com"\` — altså den fejl, porten er skrevet til at finde; nu ordbøger. (d) Min egen \`wrong_repo\`-kontrol blev **bygget og kasseret**: den krævede en håndskrevet liste over søskenderepos og fejlede på \`mahope/deskuptime\`, som er en ægte del af familien. \`no_domains\` dækker samme fejl uden nogen liste. En syvende fejlform kom fra en eksisterende port: \`check_license_clients.py\` erklærede min nye fil for en licensklient, fordi den nævner licens-API'en i en kommentar om hvorfor den ikke gør det — samme forvirring som opgave 20, lukket via portens egen \`NOT_CLIENTS\`.`
...
- `BASELINE`: `main@f254c5c`
- `LAST_BRANCH`: `ceo/unbuybar-pris-port`
- `NEXT_TASK`: `43 — 199 krydsside-ankere er aldrig valideret. Se opgave 43.`
...
- `TASK_ATTEMPTS`: `18: 1/1., 19: 1/1., 20: 1/1., 21: 1/1., 22: 1/1., 23: 1/1., 26: 1/1., 27: 1/1., 28: 1/1., 29: 1/1., 30: 1/1., 31: 1/1., 32: 0/0 (kræver Mads), 33: 1/1., 34: 1/1., 35: 1/1., 37: 1/1., 38: 1/1., 39: 1/1., 40: 1/1., 41: 1/1., 42: 1/1.`
...
- `SLIP` (opgave 42, rettet ikke): `Jeg commitede deploynotens merge-SHA direkte på main. After første merge (3e74f11) stod jeg stadig på main, så `git add IMPLEMENTATION_PLAN.md && git commit` lagde 5804f90 dér i stedet for på ceo/unbuybar-pris-port, og det efterfølgende `git merge` sagde "Already up to date", fordi branchens commit allerede var en stamfar. AGENTS.md forbyder det, og opgave 26 har præcis samme slip. Indholdet er rigtigt og pushet, og `git push --force` er forbudt, så det kan ikke ryddes pænt. Følgen er én ekstra commit på main for én opgave. Næste iteration må ikke læse 3e74f11 + 5804f90 som to planændringer — de er én kodeændring og én deploynote.`
...
**Acceptkriterier:** et svar fra `/api/download` med 503 må ikke resultere i et link på
`/thanks`; `tests/thanks-page.test.mjs` udvidet med det tilfælde.
...
### 43. UFÆRDIG — 199 krydsside-ankere er aldrig valideret
