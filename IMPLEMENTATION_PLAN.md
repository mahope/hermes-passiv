...
- `ITERATION_ID`: `retired-downloads-2026-09-26`
- `STATE`: `Opgave 28 FÆRDIG — researchiterationen begyndte med at gen-teste opgave 27s åbne `DEPLOY`-note, og svaret var ja: `clean-copy-firefox-v1.5.3.zip` svarer stadig **200** på cleancopy.tools (23344 byte). Jeg pakkede den ud, og den **indeholder stadig den falske README** — linjen *“No network requests — nothing leaves your browser”* — sammen med et badge på `version-1.4.1` i et arkiv der hedder 1.5.3. **Fund 1 — opgave 27s rettelse holdt i kilden og ingenting andet.** Filen er slettet i git, den er i ingen dist, og den ligger i ingen kilde, så alle 40 gatestræk var grønne på præcis det fejl, opgaven handlede om. **Årsagen er ikke en fejl i kilden:** Cloudflare Pages *fjerner ikke slettede assets* — den gamle fil bliver liggende i CDN'en og serveres. Den gruppe kunder, der rammer den værste fejl (en der henter klienten og tror på privatlivsteksten), fik den gamle fil. **Løsningen er en 301 i workeren**, opslået FØR `/downloads/`-ruten, så et gammelt link (blog, butiksliste, mail) lander på 1.5.4. Kildesandheden er `tools/retired_downloads.json`. **Fund 2 — min første worker-test var grøn på præcis den fejl, den var skrevet til at fange.** Test-fixturen havde ingen gammel fil i `ASSETS`, så “tilbagetrukket arkiv tælles ikke” var sand uden at prøve noget. Fixturen serverer nu den gamle fil med netop den falske tekst — det er den virkelige tilstand — og fire mutationer fanges alle: reglen væk (72/77), tabellen tom (72/77), selvrefererende 301 (75/77), reglen for *efter* `/downloads/`-ruten (72/77). Sidstnævnte er den vigtige: den skulle have ladet `handleDownload` tælle den gamle fil som et download og serveret den videre. **Fund 3 — min egen port havde to fejl, som selftesten afslørede fordi den *krævede* dem.** (a) Kildefilene blev fundet via en `glob` på modulniveau, altså frosset da porten blev skrevet — selftestens probe-fil blev aldrig læst, og scenariet “vores side sender kunden i den gamle fil” stod som grønt, fordi porten aldrig kiggede. Samme stumme kontrol som opgave 17 fund 3. (b) Min `expect()` matchede på så løse former, at den fejlede på de rigtige fund, og jeg ville have justeret *reglerne* ind i passende former i stedet for at rette forventningerne. Nu har hver fejl et navngivet regel-id, og selftesten matcher på det. **Bevis:** 11/11 selftest-kontroller, 0 fejl på den rigtige kode. **Den afgørende kontrol:** `check_live_sitemaps.py`, som kører efter hver deploy, kræver nu at en tilbagetrukken sti **ikke** svarer 200 — den eneste kontrol der kan se CDN'en, alt i `tools/` læser repoet og dist, og begge var rene. **Konvertering:** 1.5.4-arkivet behøver ingen ny licens, og 301'en gør gamle links brugbare igen i stedet for at sende kunden i en død fil. Chrome-extensionen er urørt på 1.5.3, fordi den ikke er ændret.`
...
- `NEXT_TASK`: `29 — researchiteration: køen er tom igen. Først den påstandsklasse fund 1 opdagede: de øvrige offentlige tekster ("no phone home", "nothing leaves", privacy-afsnit på de fire domæner) er ikke geret af nogen port. Derefter en konverteringsmulighed fra reports/weekly/. Bemærk at opgave 28 viste, at “filen er væk fra git” aldrig betyder “filen er væk for kunden” — samme spørgsmål bør stilles til hver anden publiceret fil. 24 og 25 er BLOCKED: kræver Mads.`
- `PLAN_COMMIT`: `03751ae` — kode + plan i samme commit, ingen ren plan-commit`
- `BASELINE`: `main@63d83a0`
- `LAST_BRANCH`: `ceo/retired-downloads`
- `TASK_ATTEMPTS`: `18: 1/1., 19: 1/1., 20: 1/1., 21: 1/1., 22: 1/1., 23: 1/1., 26: 1/1., 27: 1/1., 28: 1/1.`
...
- `DEPLOY` (opgave 28, åben): `VERIFICÉR DEPLOY: 301 på /downloads/clean-copy-firefox-v1.5.3.zip + de to nye gatestræk <merge-sha> 26/9` — GitHub Actions deployer med det samme, fordi `site/_worker.js` og `tools/**` er i path-filteret. Verificér **indhold**: den gamle sti skal svare 301 med Location `.../clean-copy-firefox-v1.5.4.zip` (curl -sI; HTTP 200 på 1.5.4 er ikke bevis), og `build-info.json` skal bære merge-SHA'en på de tre domæner. CI-jobbet “Tjek produktion” gør det selv og dræber deployen, hvis stien stadig svarer 200.`
