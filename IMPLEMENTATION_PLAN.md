...
- `DEPLOY` (opgave 43, åben): `VERIFICÉR DEPLOY: 37 rettede ankerlinks + duplikat-id-fixen d1ef1a4 26/9` — merge-SHA `d1ef1a4`, kørsel `36219021351`. Deploy sker med det samme (dette repo deployer fra `main`), så verificér **indhold**, ikke HTTP 200: (1) `https://cleancopy.tools/da/` skal have **0** fund af `mahope.tools/da/#install` og mindst 1 af `href="/da/#install"`, og `#install` skal findes på den side; (2) `https://mahope.tools/guides/platforms` skal have **0** duplikattributter på `<main>` — den skal være `<main id="main">` med `id="platforms"` på næste element — og `#platforms`-linket skal være der; (3) `https://mahope.tools/da/guides` skal have 0 fund af `/da/#tools`; (4) `https://cleancopy.tools/da/blog/html-til-markdown-cli` skal have 0 fund af `/clean-copy#cli`. Alle tre domæners `build-info.json` skal bære `d1ef1a4`.
