...
- `DEPLOY`: `VERIFICÉR DEPLOY: /api/stats har sin egen nøgle, mailnøglen er kun fallback 34e869f 26/9 ~14:5x (kodecommit c58c3e2)` — GitHub Actions deployer med det samme (\`site/**\`, \`tools/**\`, \`tests/**\` er i path-filteret). Denne iteration rører \`site/_worker.js\`, så **den eneste reel risiko er at en kunde eller rapporten låses ude af /api/stats**. Verificér på **indhold**, ikke HTTP 200:
   - \`https://mahope.tools/build-info.json\` skal bære **34e869f** på alle tre domæner, og \`sitemap_count\` skal være **252** (uændret).
