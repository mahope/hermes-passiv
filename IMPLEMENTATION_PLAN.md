...
- `DEPLOY`: `(opgave 53, ÅBEN): `VERIFICÉR DEPLOY: vidne-reglen dækker nu også de tre node-testsuiters <merge-sha> 26/9` — GitHub Actions deployer med det samstoende (`tools/**` og `tests/**` er i path-filteret). Beviset er at `gate`-kørslen bliver GRØN: før opgave 52 dræbte præcis denne klasse alle deploys. `build-info.json` på alle tre domæner skal bære merge-SHA'en.`
