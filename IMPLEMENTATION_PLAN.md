...
- `NEXT_TASK`: `34 — researchiteration. Køen er tom igen, så undersøg de tre øvrige domæner og de otte produkter for den næste fejlklasse i købsrejsen. Kandidater at undersøge: (a) `/activate/`-siden for hvert produkt — aktiveringsvejen er missionskritiske og kun dækket af `check_license_clients.py`, ikke af en ende-til-ende-test; (b) de syv `kind: 'download'`-produkter: `/thanks` renderer dem nu, men ingen test dækker at de *files* de lister findes i KV; (c) `reports/weekly/2026-39.json` har `"traffic": {}` og mangler `schema_version` — uge 40 sammenligner mod en fil fra en ældre scriptversion.`
- `PLAN_COMMIT`: `— denne iteration: kode + plan i samme commit, ingen ren plan-commit`
- `BASELINE`: `main@daed5d8`
- `LAST_BRANCH`: `ceo/thanks-donation`
- `TASK_ATTEMPTS`: `18: 1/1., 19: 1/1., 20: 1/1., 21: 1/1., 22: 1/1., 23: 1/1., 26: 1/1., 27: 1/1., 28: 1/1., 29: 1/1., 30: 1/1., 31: 1/1., 32: 0/0 (kræver Mads), 33: 1/1.`
...
### 32. BLOCKED: kræver Mads — den samme hule tabel på EAA-scannerens blogside

`site/blog/eaa-compliance-scanner-desktop.html` har fire tomme celler i sin Free/Pro-tabel.
**Bevidst ikke rettet i opgave 31:** EAA Scanner Pro har ingen `product_key` (❓ punkt 5), så
værdierne kan ikke udledes af koden, og siden sælger. Se `❓ Til Mads` punkt 12.

Opgave 33 har bekræftet at dette er **det eneste tilbageværende eksempel på fejlen**: en
scan af alle 114 synlige sammenligningstabeller i `site/` giver 5 tabeller, hvoraf 4 er
fyldte og kun denne har tomme celler. Blokeringen er Mads' beslutning, ikke en mangel i
mit arbejde, så den tæller ikke som et mislykket forsøg.
...
### 34. UFÆRDIG — researchiteration: næste fejlklasse i købsrejsen

Se `NEXT_TASK` for de tre kandidater.
