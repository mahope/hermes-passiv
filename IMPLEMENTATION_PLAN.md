...
- `RESULT` (opgave 15): `site/sitemap.xml` var en **tom urlset** (153 bytes, nul `<url>`), tracked og aldrig læst. `build_sites.py:155` har den i `SKIP_NAMES`, og sitemapperne skrives til `dist/<domaene>/sitemap.xml` (build_sites.py:1113). `tools/check_sitemaps.py` læser kun dist. Så 21 generatorer skrev URL'er ind i en fil, der aldrig blev publiceret, og skrev "Sitemap updated: N URLs" bagefter — præcis samme fejlform som de 18 "broken references" i opgave 10: en tælling der så rigtig ud og rettede ingenting.
  ...
  **Fund 5 — min egen port døde i CI med det samme den var grøn lokalt, og det var femte gang.** Kørsel `36194467339` fejlede på `auditedwp-src/site/deploy.sh` — præcis fejlformen `check_license_clients.py` fik i opgave 11 (fund 5), fordi CI checkouter auditedwp ved siden af workspace så den ligger inde i `ROOT` og aldrig lokalt. Min regel var en næsten-kopi af den: den ledede et niveau op og kiggede på filens *egen forælder*, mens `.git` ligger i *forfader*directoryet. Jeg havde læst fund 5 og skrevet en regel, der ikke løste den. Beviset er nu to selftest-scenarier — et eksternt checkout springes over, og vores egen kode fanges stadig — plus en read-onlyprove i det rigtige repo med et checkout ved et navn, der *ikke* står på nogen liste, så det er reglen og ikke listen der virker.
...
- `RESULT` (opgave 14): `tools/check_versions.py` erklærer otte produkter med hver **én** kanonisk fil og fire checks: sandheden findes, ingen spejling lyver (lockfil, `__version__`, Obsidians `versions.json`), det publicerede arkiv findes på den version og **intet** gammelt arkiv må ligge i samme familie, og `site/downloads.html` skal nævne præcis den version. Det er samme fejlform som de 18 "broken references" i opgave 10 gjorde til en port: værdi med to hjemsteder er ikke et versionsproblem, det er et kildefejl, fordi spørgsmålet "hvad er versionen" så er en mening og ikke et faktum.
  ...
  **Fund 1 — de seks fund var reelle, og tre af dem var PUBLICERET.** `manifest.json` og `versions.json` i repo-roden sagde 1.0.1 om Obsidian-pluginet, mens `obsidian-plugin/manifest.json` sagde 1.0.10 og arkivet hed `clean-copy-obsidian-v1.0.10.zip`. Roderne lå fra den første mono-repo-commit, bruges af intet build, og var den første fil et menneske læser, når det spørger efter versionen. De er slettet, og `FORBIDDEN_SOURCES` gør det umuligt at de kan komme tilbage uden at porten siger det. De tre øvrige var `eaa_scanner-1.1.0-py3-none-any.whl`, `eaa_scanner-1.1.0.tar.gz` og `eaa-scanner-desktop-src-1.3.0.zip` i `site/downloads/`: filer ingen side linkede til, som en kunde alligevel kunne hente, og som derfor var gammel kode uden at nogen vidste det. Fjerne dem er den eneste rettelse — at rette teksten ville have ladt URL'en stå. (+ `scanner/npm/eaa-scanner/mahope-eaa-scanner-1.0.0.tgz`, et 1.0.0-tarball inde i 1.2.0-pakkemappen.)

  **Fund 2 — porten havde to fejl, og den positive kontrol fandt begge.** Først forsøgte den at læse `packages.""` i lockfilen som en tom sti, så den ignorerede nøglen og erklærede `desktop/package-lock.json` uden version — i en mutationstest der *skulle* finde netop den fejl. Så skrev mutationen "ugyldig JSON" ved at erstatte `1.5.3` med `1.5.3,`, hvilket lægger kommaet *inden i* citationstegnene og giver gyldig JSON med værdien `1.5.3,` — så porten svarede "er ikke x.y.z" i stedet for "kan ikke læses som JSON". Begge blev fundet fordi selftesten kræver en grøn start, ikke fordi nogen læste koden. Det er tredje gang i dette repo (opgave 11, 13, 14) at en gate uden positiv kontrol på sin egen logik viser sig at være teater.

  **Fund 3 — min egen privacy-rettelse brød en anden port, og det var korrekt.** `check_license_clients.py` behandler enhver fil der nævner `/api/license` som en licensklient. Jeg havde skrevet endpointet ind i prosate, fordi det er sandt og nyttigt, og gaten sagde: en side der nævner et licenskald skal være opført med sit `product_key`. Retten er ikke at føje en dokumentationsside ind som klient — det ville svække den port — men at skrive "a license endpoint on mahope.tools" i stedet. **Fund 4 — `desktop/package.json` kan ikke gates fra deploy-sites, og det er ikke en fejl jeg måtte rette.** `test_deploy_workflow` forbyder med vilje at en desktop-ændring udløser sites-deployen, fordi desktop udgives af `build-desktop.yml`. Men `check_versions` læser netop `desktop/package.json` og `package-lock.json`, så en desktop-versionbump er den ENeste versionsdrift gaten ikke fanger automatisk. Det er noteret her frem for løst ved at slække en eksisterende regel: **det næste skridt er at give `build-desktop.yml` et `gate`-job med `check_versions`**, ellers er desktop-locket ubevidnet i CI.

  **Privacy gjort ærlig, ikke dækkende.** Den gamle tekst sagde "we do not collect, store, or transmit your personal data" og fandt så en side senere, fordi den skrev om anonymke sidevisninger, og den dækkede `hermes-passiv.pages.dev`, som ikke er et live-domæne. Den nye tekst fortæller hvad der faktisk sker, hver del med egne oplysninger: sidevisningstællinger uden identifikator med 90 dages udløb, en 2-timers hash til rate limiting (og at `Do Not Track: 1` dropper kaldet før der skrives noget), **ventelisten, som gemmer mailadressen du skriver i 365 dage** — den stod slet ikke før, selvom den er den eneste reelle persondata her — licensnøglen og et tilfældigt device-id der sendes online, og **at BugBottle-demoens indbakke er offentlig læsbar**, så man ikke skal skrive følsomt i en offentlig rapportform. Det godkender intet nyt datagrundlag; det beskriver det, der var.

  **De seks roddokumenter er nu arkiv.** `STATUS.md` var fra 26. august, erklærede iterationen PAUSET og førte en "Næste iteration"-liste med Lemon Squeezy som aktiv betaling; den er skrevet om til live-fakta, produkter, ærlige tal og de ting Mads afventer. `DECISION.md` gjorde det samme og dokumenterer nu de fire beslutninger (Stripe, open-core, universal sourcing, drift uden Mads) med de forbehold de blev truffet under. `BUDGET.md` lovede næste udgift "før Mads siger ja til KDP/**Gumroad**-konti" og pegede på 35 kr brugt af 1.000 — beløbet er uændret og korrekt, kun formuleringerne om fremtidige konti er fjernet og de gratis niveauer er dokumenteret som gratis. `BUILD.md` og `RESEARCH.md` beholder deres indhold og har fået en arkiv-præamble, fordi de er faktuelle iterationer med dato, ikke planer. Root-README skrev "No network access. No telemetry." for Obsidian-pluginet, hvilket er **falsk** siden Pro-licensen tjekkes online mod `mahope.tools` — præcis den samme løgn som opgave 2 rettede på DeskUptime, og den er nu erstattet af en beskrivelse af hvad der sendes (nøgle + device-id) og at pluginet virker offline.
