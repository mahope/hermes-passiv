...
- `DEPLOY` (opgave 21, lukket): `DEPLOY OK 60e30d9 26/9` — kørsel `36202790956`: `gate` grøn (38 steps, `check_versions --self-test` 31 mutationer + 2 negative kontroller) + tre grønne deploys (cleancopy.tools, deskuptime.com, mahope.tools). Committen rørte kun `tools/check_versions.py`, så domænerne er uændrede — bekræftet på indhold: `site-icons-1.0.0.tar.gz` er byte-uændret (sha256 `cbafbd98…`) og `clean-copy-obsidian-v1.0.10.zip` (12879 bytes) er byte-identisk med repoet.`
