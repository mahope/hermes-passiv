...
- `DEPLOY`: `VERIFICÉR DEPLOY: købssider har nu en hard gate for indgang <merge-sha> 26/9` — GitHub Actions deployer med det samme (\`site/**\` og \`tools/**\` er i path-filteret). Denne iteration rører **kun \`tools/check_stripe_ctas.py\`**, så intet domæne ændrer indhold — verificér derfor på indhold, at intet er ændret:
