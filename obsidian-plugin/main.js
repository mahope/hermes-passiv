...
var DEFAULT_SETTINGS = {
  licenseKey: '',
  proActive: false,
  licenseCheckedAt: 0,  // ms timestamp of the last positive license check
  expiresAt: null,
...
// The inlined module above publishes itself as a CommonJS export under Node
// and as a global in the Obsidian sandbox; take whichever one it produced.
var CleanCopyLicense = (function () {
  if (typeof module === 'object' && module && module.exports && module.exports.PRODUCT) return module.exports;
  return (typeof self !== 'undefined' ? self : this).CleanCopyLicense;
})();

var LICENSE_API = CleanCopyLicense.API_BASE;
...
module.exports = (function () {
  var Plugin = /** @class */ (function () {
    ...
    Plugin_.prototype.licenseExchange = async function (endpoint) {
      // One HTTP exchange against the license API, reduced to what
      // CleanCopyLicense.decide() needs. A thrown request is status 0, i.e.
      // "no answer", which is a server error — not a lost license.
      var key = CleanCopyLicense.normalizeKey(this.settings.licenseKey);
      try {
        var res = await licenseRequest(endpoint, CleanCopyLicense.payload(key, this.settings.deviceId));
    ...
    Plugin_.prototype.applyLicenseDecision = async function (exchange) {
      var d = CleanCopyLicense.decide({
        status: exchange.status,
        data: exchange.data,
        checkedAt: this.settings.licenseCheckedAt || 0,
        expiresAt: this.settings.expiresAt || ''
      });
      this.settings.proActive = d.active;
      this.settings.licenseCheckedAt = d.active ? d.checkedAt : 0;
      if (d.active) this.settings.expiresAt = d.expiresAt || this.settings.expiresAt;
    ...
    Plugin_.prototype.activateLicense = async function () {
      var key = CleanCopyLicense.normalizeKey(this.settings.licenseKey);
      if (!CleanCopyLicense.isKeyFormat(key)) return false;
      var d = await this.applyLicenseDecision(await this.licenseExchange('activate'));
      new obsidian.Notice(d.active
        ? (d.cached ? 'Clean Copy Pro active (' + d.message + ')' : 'Clean Copy Pro activated.')
        : d.message);
      return d.active;
    };
    ...
    Plugin_.prototype.validateLicensePeriodic = async function () {
      ...
      if (!CleanCopyLicense.isKeyFormat(this.settings.licenseKey)) {
        this.settings.proActive = false;
        this.settings.licenseCheckedAt = 0;
        await this.saveSettings();
        return;
      }
      var d = await this.applyLicenseDecision(await this.licenseExchange('validate'));
      if (!d.active) new obsidian.Notice('Clean Copy Pro: ' + d.message);
      // A server outage keeps Pro for at most seven days after the last
      // successful check, so a paying customer is never locked out by a
      // hiccup — and a revoked key still loses Pro on the first answer.
    };
